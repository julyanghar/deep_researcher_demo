...
> 基于三份实测报告([summary.md](summary.md) / [overlap-replay.md](overlap-replay.md) / [per-request-speedup-variance.md](per-request-speedup-variance.md))的优化设计。只讲方案与取舍,均未动手;每个方案附"怎么验证成不成"。
...
**净收益测算**(每题的 LLM decode 段,拿实测均值直接算):

| | baseline | 全开 suffix | 变化 |
|---|--:|--:|--:|
| summary 段(6 次 × 14.7s→16.3s)| 88.2s | 97.8s | +9.6s(亏)|
| report 段(80.1s→43.4s)| 80.1s | 43.4s | **−36.7s(赚)** |
| 两段合计 | 168.3s | 141.2s | **净省 ~27s / 题(~16%)** |

**为什么敢全开**:report 是 decode 大头且恒赚,分析型 summary 各亏 ~7-18% 但绝对量小,净账是正的。
...
## 建议路线图
1. **L0 先上**(零代码):全开 suffix + 并发压测找红线 → 真实负载净收益报告;
2. **L3 小实验**(一次 prompt 改动 + 20 题 A/B):性价比最高的"让 summary 翻正"路径;
3. **L2 离线部分先跑**(零 GPU):拿 1812 条已有 harvest 打标、训预测器、模拟路由收益——数据说话后再决定要不要上双池;
4. L4/LMCache 兼容测试:各一次 server 实验的成本,插空做。

## 一句话
**先全开吃 report 的确定性红利(净省 ~16%/题),再用免费的离线数据决定要不要为 summary 做路由;真正的杠杆不在调参(已证明无用),在"让该开的开、不该开的躲开"。**
