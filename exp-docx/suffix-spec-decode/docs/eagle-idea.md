...
> **🔴 2026-07-09 加速实测追补:eagle3 不止接受率低,是净减速(summary 0.92-0.97×、report 0.86-0.88×,比不投机还慢);suffix 同条件 1.89×/2.41×。见 §加速实测。**
> **🔵 2026-07-10 下一步:域内继续训练方案已成稿(SpecForge warm-start AngelSlim head + 自产 on-policy harvest 数据 + 预注册判据 P0 15%/P1 20%/P2 35%)——见 [eagle3-domain-training-plan.md](../../eagle-spec-decode/eagle3-domain-training-plan.md)。**
...
### ★ 加速实测(2026-07-09 补):eagle3 不是"加速少",是净减速

接受率之外补测**实际加速**:同 TP4/GPU 0-3、同批真实 prompt(20 summary + 10 report,取自 40 题 c2_spec40 harvest)、temp=0、**串行回放**(decode 计时无并发污染),server 侧 `timing.decode_s` 算 decode tok/s。物证 `~/modify-code-runs/eagle3-test/speedup_*.{log,json}`、`measure_speedup.py`。

| drafter | summary tok/s(加速×) | report tok/s(加速×) | summary 接受率 | report 接受率 |
|---|---|---|---|---|
| vanilla(cuda-graph,基线) | 42.6(1.00×) | 41.5(1.00×) | — | — |
| **suffix**(enforce-eager+traj) | **80.5(1.89×)** | **100.2(2.41×)** | 43.7% | 60.2% |
| eagle3-RedHat(cuda-graph) | 41.5(0.97×) | 36.5(**0.88×**) | 13.6% | 19.3% |
| eagle3-AngelSlim(cuda-graph) | 39.1(0.92×) | 35.6(**0.86×**) | 10.7% | 17.7% |

- **eagle3 两个 head 都是净减速(0.86-0.97×)**:接受率 11-19% 太低,drafter 每步的真实开销(3 次草稿前向 + hidden state 抽取)吃掉省下的步数还倒贴——**report 反而比不投机慢 12-14%**。
- **成本已解剖到成分**(k=1/2/3/5 扫描 + eager 对照 + 分段账本,拟合残差 ±0.1ms):每步 = 基线 + **k无关固定费 8.2/16.6ms**(强 ctx 敏感,drafter 前向占 propose 的 ~70%)+ **0.80-0.90ms×k**(vs suffix 每位置 0.62);**eagle3 不交混合边界税**(k 固定形状稳,eager 反慢 4%);k 扫遍也过不了盈亏线(AL 饱和 1.42/1.64);**2× 需接受率 63%**。详见 [examine-spec-tax/eagle3-cost-anatomy.md](examine-spec-tax/eagle3-cost-anatomy.md)。
- **suffix 背着 enforce-eager 的 handicap 还赢 1.89-2.41×**(草稿=CPU 树查找近乎免费 + 接受率 44-60%)→ 结论更硬:**本 workload 上 suffix 全面碾压现成 eagle3,后者连"部署了不亏"都不成立**。
- 诚实边界:①执行模式不同(suffix=eager、eagle3/vanilla=cudagraph),方向对 suffix 不利,结论保守;②跨 server 输出**非字节一致**(TP 浮点+执行模式数值差→greedy 路径分叉,0/30),但生成量 ±6%(report 全顶 2500 cap),tok/s 速率口径对量差稳健;③suffix 臂带 traj 写盘开销,同样是保守方向。
